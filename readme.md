...
The biggest performance problem is that the structs are too large. Even if a sorter is on the other side of the star cluster, the sorter's animation-relevant data still has to be loaded in when the game updates the sorter.
...
The large structs the game uses contains a lot of internal padding between its fields. Adding the attribute `[StructLayout(LayoutKind.Sequential, Pack=1)]` to each of the game's large structs should improve performance and reduce memory usage by removing padding in the structs. For some structs this can halve their size.

### Assembler/Labs linear data access
Each assembler and lab contains multiple small arrays for things their recipe requires, how much it requires, what it produces etc. Access to all these smaller arrays, acrosds multiple assembler/labs, can effectively be seen as random data access. Although the arrays within a single assembler are allocation close to each other time wise, they are so small that they fill in the gaps between other allocations. This causes big issues because there is so many of these smaller arrays and they are accessed so often. To improve data access, Weaver has flattened these small arrays into one array. This improves data access as the data access pattern becomes fully sequential. Flattening the arrays also improves data access for sorters because their mostly sequential access to assemblers and labs means mostly sequential access to their flattened data.

### Belt linear data access
DSP splits belts on a planet up into three parts: The individual belts, cargo paths and a cargo container. Each belt represents a belt on a single tile. A cargo path represents the path items moves on connected belts, this is what is responsible for actually moving items on the belts. Each planet has a single cargo container that stores all information about items on belts for that planet. This is presumably an optimization the devs have made to improve rendering performance. Cargo paths do not store the items themselves, instead they store an index to the items location in the cargo container. Cargo paths move this index around instead of moving the item itself. 

Each index takes up 28bits of space. Storing an items id, stack size and proliferation only needs 24bits of space. Weaver removes usage on the cargo container and instead stores each items data in the cargo path in place of the index. The improvement here is removing the indirection to get an items data. This improves sorter update performance but slightly decreases belt buldings update performance. Data access time is improved but it's slightly more expensive to update an items data because of how it's stored in the cargo path.
...
* Belts
	...
	* Items are stored in each `CargoPath.buffer` array instead of being stored in `CargoContainer`.
* Spray Coaters
	* Parallelized logic on a sub-factory basis.
	* Optimized data format.
