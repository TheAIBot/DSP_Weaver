...
internal sealed class EjectorExecutor
{
    ...
    private short[] _optimizedBulletItemId;
    private int[] _ejectorNetworkIds;

    public void GameTick(PlanetFactory planet, long time, int[] consumeRegister)
    {
    ...
    private static uint InternalUpdate(ref EjectorComponent ejector, float power, long tick, DysonSwarm swarm, AstroData[] astroPoses, short optimizedBulletId, int[] consumeRegister)
    {
        ...
        if (ejector.runtimeOrbitId == 0 && !ejector.needFindNextOrbit)
        {
            ...
            {
                ejector.time = (int)((long)ejector.time * (long)ejector.coldSpend / ejector.chargeSpend);
                ejector.direction = -1;
            }
            if (ejector.direction == -1)
            {
        ...
        if ((double)vector3.y < 0.08715574 || vector3.y > 0.8660254f)
        {
        ...
        if (ejector.direction == 1)
        {
            ...
            {
                ejector.fired = true;
                SailBullet bullet = default(SailBullet);
