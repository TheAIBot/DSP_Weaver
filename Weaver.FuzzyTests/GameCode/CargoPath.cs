...
public class CargoPath
{
    ...
    public CargoPath outputPath;

    public int outputIndex = -1;

    public int outputPathIdForImport;
    ...
    public const int kCargoLength = 10;

    public const int kCargoLengthMinusOne = 9;

    public const int kCargoLeftMargin = 4;

    public const int kCargoRightMargin = 5;

    public const byte kCargoHead = 246;

    public const byte kCargoRear = byte.MaxValue;

    public const byte kCargoSign = 250;

    public const int kInsertSqueezeRange = 2880;

    public const int kBuckleBufferLength = 9;

    public const int kMaxCargoFlowSpeedPerSecond = 120;
    ...
    public bool TryInsertCargo(int index, int cargoId)
    {
        ...
        if (flag3)
        {
            ...
            throw new InvalidOperationException("断言失败：插入货物逻辑有误");
        }
        return false;
    }
    ...
    public Cargo PickCargoAtIndexDirect(int index)
    {
        ...
        Cargo result = cargoContainer.cargoPool[num];
    ...
    public void ClearItemDirectly(int startIndex, int endIndex)
    {
        lock (buffer)
        {
            if (startIndex < 0)
            ...
            while (num3 <= endIndex)
            {
                if (buffer[num3] >= 246)
                {
                    num3 += 250 - buffer[num3];
                    int index = buffer[num3 + 1] - 1 + (buffer[num3 + 2] - 1) * 100 + (buffer[num3 + 3] - 1) * 10000 + (buffer[num3 + 4] - 1) * 1000000;
                    Array.Clear(buffer, num3 - 4, 10);
                    num3 += num2;
                    if (updateLen < num3)
                    {
                        updateLen = num3;
                ...
                {
                    num3 += num;
                    if (num3 > endIndex && num3 < endIndex + num)
                    {
                        num3 = endIndex;
                    }
                }
    ...
    public void RemoveCargosInSegment(int begin, int end)
    {
        lock (buffer)
        {
            ...
            while (num2 <= end)
            {
                ...
                throw new InvalidOperationException("断言失败：buffer数据有误");
            }
            Array.Clear(buffer, begin, end - begin + 1);
        }
    }

    public bool GetCargoAtIndex(int index, out Cargo cargo, out int cargoId, out int offset)
    {
        cargo.stack = 1;
        cargo.inc = 0;
        cargo.item = 0;
    ...
    public void Update(long time)
    {
        if (outputPath != null)
        {
            ...
            byte[] array = ((id > outputPath.id) ? buffer : outputPath.buffer);
            lock ((id < outputPath.id) ? buffer : outputPath.buffer)
            {
                lock (array)
                {
